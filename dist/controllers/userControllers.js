...
export const forgotPassword = async (req, res) => {
    try {
        ...
        const link = `${process.env.CLIENT_URL}/reset-password/${user._id.toString()}/${secretToken}`;
        await sendEmail({
            from: "convoconnect@gmail.com",
            to: email,
            subject: "Forgot Password?",
            html: forgotPasswordTemplate(link),
            text: `Use the link to reset your ConvoConnect account's password.\n Visit: ${process.env.CLIENT_URL}/reset-password/${user._id.toString()}/${secretToken} to reset your password.`,
        });
        return res.status(200).json({
            success: true,
            message: "Email Sent.",
